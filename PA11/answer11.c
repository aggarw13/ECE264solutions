...
int isValidState(const char * state)
{
  if(state == NULL)
    return 0;
  if(strlen(state) == SIDELENGTH * SIDELENGTH)
    {
      char * buffer = malloc(sizeof(char)*(SIDELENGTH * SIDELENGTH + 1));
      strcpy(buffer,state);
      qsort(buffer,16,sizeof(char),comparechar);
...
{    
  ...
  if((ncol >= 0)&&(ncol < SIDELENGTH)&&(nrow >= 0)&&(nrow < SIDELENGTH))
    {
      char temp = state[row*SIDELENGTH + col];
      state[row*SIDELENGTH + col] = state[nrow*SIDELENGTH + ncol];
      state[nrow*SIDELENGTH + ncol] = temp;
      return TRUE;
    }
  return 0; 
}
...
MoveTree * MoveTree_create(const char * state, const char * moves)
{
  ...
  if(node != NULL)
    {
      node -> state = malloc(sizeof(char)*(SIDELENGTH * SIDELENGTH + 1));
      node -> state[SIDELENGTH*SIDELENGTH] = '\0';
      node -> moves = malloc(sizeof(char)*(MAX_SEARCH_DEPTH + 1));
      node -> moves[MAX_SEARCH_DEPTH] = '\0';
			   ...
			   const char * moves)
{
  ...
  MoveTree * prev = MoveTree_find(node, state);
  if(prev != NULL)
    {
      if(strlen(prev -> moves) > strlen(moves))
	strcpy(prev -> moves, moves);
      return node;
    }
 ...
 */
void movehelper(MoveTree * root, char * state,int moves, char * movelist, int ind)
{
  if(ind == moves)
    return;
  ...
  for(i = 0; i <= 3; i++)
    {
      char * statedup = malloc(sizeof(char) * (SIDELENGTH * SIDELENGTH + TRUE));
      statedup[SIDELENGTH * SIDELENGTH] = '\0';
      strcpy(statedup, state);
      if(move(statedup, moveb[i]))
	{
	  movelist[ind] = moveb[i];
	  movelist[ind + 1] = '\0';
	  // processMoveList(statedup, movelist);
	  MoveTree_insert(root, statedup, movelist);
	  movehelper(root, statedup, moves, movelist, ind + 1);
	  //  movelist[ind + 1] = '\0'; 
	  free(statedup);
	}
      else
...
{ 
  ...
  movelist[n_moves] = '\0';
  char empty[] = "";
  MoveTree * root = MoveTree_create(state, empty);
  movehelper(root, state, n_moves, movelist, 0);
  free(movelist);
  return root;
}
...
char * solve(char * state)
{
  ...
  char * solved = malloc(sizeof(char) * (SIDELENGTH* SIDELENGTH + TRUE));
  solved[SIDELENGTH * SIDELENGTH] = '\0';
  strcpy(solved,FINAL_STATE);
  MoveTree * node = MoveTree_find(root, solved);
  free(solved);
  if(node != NULL)
    {
      char * movelist = strdup(node -> moves);
