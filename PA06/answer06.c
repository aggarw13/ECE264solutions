...
struct Image * loadImage(const char* filename)
{
  ...
  struct Image * imagedat = malloc(sizeof(struct Image)); 
  if(fh != NULL)
    {
      //struct ImageHeader header;
      //  struct ImageHeader * header = malloc(sizeof(struct ImageHeader));
      if((fread(header,sizeof(struct ImageHeader),1,fh) != 1) || (header -> magic_bits != ECE264_IMAGE_MAGIC_BITS) || (header -> width <= 0) || (header -> height <=0))
       {
	 free(header);
	 free(imagedat);
	 fclose(fh);
     ...
     char * arr = malloc(sizeof(char) * header->comment_len);
     if((header -> comment_len > 0) && (fread(arr,sizeof(char) * header->comment_len,1,fh) == 1) && (arr[header->comment_len - 1] == '\0'))
	   {
	     uint32_t i = 0;
	     uint8_t * pixeldat = malloc(sizeof(uint8_t)* header->width * header->height);
	     if(pixeldat != NULL)
	       {
	     // fread(pixeldat,sizeof(uint8_t) * header->width * header->height,1,fh);
		 ...
		 //struct Image * imagedat = malloc(sizeof(Image));
		 imagedat-> width = header->width;
		 imagedat-> height = header->height;
		 imagedat-> comment = malloc(sizeof(char)*header->comment_len);
		 strcpy(imagedat->comment,arr);
		 imagedat->data = pixeldat;
		 fclose(fh);
		 free(header);
		 free(arr);
		 /* free(pixeldat); */
...
void linearNormalization(struct Image * image)
{
  if(image != NULL)
    {
      int i;
      uint8_t max = image->data[0],min = image->data[0];
      for(i = 1; i < image -> width * image -> height; i++)
	{
      ...
      //printf("\n max = %d , min = %d data[0] %d", max, min, image->data[0]);
