...
char * * readString(char * filename, int * numString)
{
  ...
  if(fh == NULL)
    ...
    { 
      int noLines = 0;
      char * strt = malloc(sizeof(char)*MAXIMUM_LENGTH);
      *numString = 0;
      while(fgets(strt,MAXIMUM_LENGTH,fh)!= NULL)
	{
 	  ++noLines;
	}
      *numString = noLines;
      ...
      do
	{
	  fgets(strt,MAXIMUM_LENGTH,fh);
	  str[i] = malloc(sizeof(char)*(strlen(strt)+ 1));
	  strcpy(str[i],strt);
	  i++;
	}while(i < *numString);
      /*      while(fgets(str[j++],MAXIMUM_LENGTH,fh) != NULL)
	{
      	}*/
      fclose(fh);
      free(strt);
...
int saveString(char * filename, char * * arrString, int numString)
{
  ...
  if(fh == NULL)
	...
	{
	  //	  if ((j < numString - 1)&&(strcmp(arrString[j],arrString[j+1]) == 0))
	  // { 
	  //  fprintf(fh,"%s",arrString[j]);
	  fprintf(fh,"%s",arrString[j]);
	  if (arrString[j][strlen(arrString[j]) - 1] != '\n')
	    {
	      fprintf(fh,"%c",newL);
	    }
	}
...
int compareINTEGER(const void * a1,const void * a2)
{
  int * p1 = (int *)a1;
  int * p2 = (int *)a2;
  if(*p1 < *p2)
...
{
  qsort(arrInteger,numInteger,sizeof(int),compareINTEGER); 
}
